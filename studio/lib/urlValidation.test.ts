...
describe('urlValidation', () => {
  describe('createURLValidation', () => {
    ...
    it('blocks dangerous protocols', () => {
      ...
      expect(validate('javascript:alert("xss")')).toContain('JavaScript and data URLs are not allowed')
      expect(validate('data:text/html,<script>alert("xss")</script>')).toContain('JavaScript and data URLs are not allowed')
    })
  ...
  describe('utility functions', () => {
    ...
    describe('sanitizeURLForDisplay', () => {
      it('removes credentials from URLs', () => {
        expect(sanitizeURLForDisplay('https://user:pass@example.com/path'))
          .toBe('https://example.com/path')
      })

      it('leaves clean URLs unchanged', () => {
        expect(sanitizeURLForDisplay('https://example.com/path'))
          .toBe('https://example.com/path')
      })

      it('handles invalid URLs gracefully', () => {
        expect(sanitizeURLForDisplay('invalid-url'))
          .toBe('invalid-url')
      })
    })
  })
