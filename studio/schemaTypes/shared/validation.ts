...
export const componentValidation = {
  ...
  // Date fields
  futureDate: (Rule: any) => Rule.min(new Date().toISOString().split('T')[0]).warning('Dato bør være i fremtiden'),
  
  // Common patterns
  phoneNumber: (Rule: any) => Rule.regex(/^[\+]?[0-9\s\-\(\)]+$/).warning('Ikke et gyldig telefonnummer'),
}
...
export const contentValidation = {
  ...
  festivalYear: (Rule: any) => {
    ...
    return Rule.min(currentYear - 1).max(currentYear + 5)
      .warning(`Festivalår bør være mellom ${currentYear - 1} og ${currentYear + 5}`)
  },
  
  // Event validation
  eventCapacity: (Rule: any) => Rule.min(1).max(10000)
    .warning('Kapasitet bør være mellom 1 og 10,000 personer'),
    
  // Artist validation
  artistGenres: (Rule: any) => Rule.min(1).max(5)
    .warning('Velg mellom 1 og 5 sjangre for artisten'),
    
  // Component-specific validation
  accordionPanels: (Rule: any) => Rule.min(1).max(10)
    .error('Accordion må ha mellom 1 og 10 paneler'),
...
export const seoValidation = {
  ...
  metaDescription: (Rule: any) => Rule.max(160).warning('Meta beskrivelse bør være under 160 tegn for SEO'),
  altText: (Rule: any) => Rule.max(125).warning('Alt-tekst bør være under 125 tegn for tilgjengelighet'),
}
...
export const a11yValidation = {
  ...
  contrastRatio: (Rule: any) => Rule.custom((value: string) => {
...
export const componentSpecificValidation = {
  // Button validation
  buttonText: (Rule: any) => Rule.required().min(1).max(30)
    .error('Knappetekst er påkrevd og må være under 30 tegn'),
    
  // Link validation
  linkText: (Rule: any) => Rule.required().min(1).max(50)
    .error('Lenketekst er påkrevd og må være under 50 tegn'),
    
  // Image validation
  imageAlt: (Rule: any) => Rule.min(1).max(125)
    .warning('Alt-tekst anbefales for tilgjengelighet (1-125 tegn)'),
    
  // Video validation
  videoTitle: (Rule: any) => Rule.max(100)
    .warning('Videotittel bør være under 100 tegn'),
    
  // Quote validation
  quoteText: (Rule: any) => Rule.required().min(1).max(500)
    .error('Sitat er påkrevd og må være under 500 tegn'),
    
  // Heading validation
  headingText: (Rule: any) => Rule.required().min(1).max(200)
    .error('Overskriftstekst er påkrevd og må være under 200 tegn'),
}
